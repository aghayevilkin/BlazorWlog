...
namespace Wlog_Server.Service
{
    public class FileUpload : IFileUpload
    {
        ...
        public bool DeleteFile(string fileName)
        {
            try
            {
                var path = $"{_webHostEnvironment.WebRootPath}\\WlogImages\\{fileName}";
                if (File.Exists(path))
                {
                    File.Delete(path);
                    return true;
        ...
        public async Task<string> UploadFile(IBrowserFile file)
        {
            try
            {
                ...
                var folderDirectory = $"{_webHostEnvironment.WebRootPath}\\WlogImages";
                var path = Path.Combine(_webHostEnvironment.WebRootPath, "WlogImages", fileName);
                ...
                var fullPath = $"{url}WlogImages/{fileName}";
                return fullPath;
